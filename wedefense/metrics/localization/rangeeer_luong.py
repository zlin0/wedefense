...
@inproceedings{zhang23v_interspeech,
    ...
    author = {Lin Zhang and Xin Wang and Erica Cooper and Nicholas Evans and Junichi Yamagishi},  # noqa
    year = {2023},
...
def _count_samples(labs,
    ...
    assert resolution + 1 == counter.shape[
        1], "ERROR: the length of the preloaded counter and the resolution is not equal"
    names = labs.keys()
    for name in names:
        lab, sco = labs[name], scos[name]
        sco = _pad_score_array(sco, lab)
        counter = _count_one_sample(counter,
                                    lab,
                                    sco,
...
def compute_rangeeer(labs,
                     scos,
                     resolution=8000,
                     counter=None,
                     minval=-2.0,
                     maxval=2.0):
    counter = _count_samples(labs,
                             scos,
                             resolution=resolution,
                             counter=counter,
                             minval=minval,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--score_index',
                        type=int,
                        default=1,
                        help="Index of the score value.")
    parser.add_argument('--score_negative',
                        action="store_true",
                        help="Score value if of the negative class")
    parser.add_argument(
        '--rttm_file',
        type=str,
        help="Path to the ground-truth timestamp annotations file "
        "in the rttm format.")
    parser.add_argument('--frame_duration',
                        type=float,
                        default=0.02,
                        help="Duration (s) of one frame.")
    parser.add_argument('--resolution',
                        type=int,
                        default=100000,
                        help="Threshold resolution")
    args = parser.parse_args()

    labs, label2id = get_rttm(args.rttm_file)
    labs = normalize_labs(labs, label2id)
    scos, minscore, maxscore = get_scores(args.score_file,
                                          score_index=args.score_index,
                                          score_duration=args.frame_duration,
                                          score_negative=args.score_negative)
    maxval = max(abs(minscore), abs(maxscore)) * 2
    minval = -maxval

    eer, threshold, margin, fpr, fnr, counter = compute_rangeeer(
        labs, scos, resolution=args.resolution, minval=minval, maxval=maxval)
    print(
        f"rangeeer={eer*100:.3f}% margin={margin*100:.3f}% threshold={threshold:.3f} minscore={minscore:.3f} maxscore={maxscore:.3f} negative_class={args.score_negative}\n"
    )
