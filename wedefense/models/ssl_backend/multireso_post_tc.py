...
class SSL_BACKEND_multireso_MaxPool1dLin_gmlp(MaxPool1d_scales):
    def __init__(self, num_scale=5, feat_dim=60, embed_dim=256, seq_len=2001, 
                 gmlp_layers=1, batch_first=True, flag_pool='ap',
                 multi_reso_active=["2","4","8","16","32","64",'utt'], Frame_shifts=[2,4,8,16,32,64]):
        super().__init__(num_scale=num_scale, feat_dim=feat_dim, embed_dim=embed_dim, 
                         flag_pool=flag_pool, multi_reso_active=multi_reso_active, 
        ...
        for i in range(num_scale):
            dim = feat_dim // pow(2,i)
            self.post_nets_seg[f"disc_{i}"] = gMLP(dim, embed_dim, seq_len // pow(2,i),
                                                   gmlp_layers=gmlp_layers, batch_first=batch_first,
                                                   flag_pool='None')
    ...
    def forward(self, x):
        ...
        for i in range(self.num_scale):
            x = self.blocks[f"{i}"](x)
            if str(2**(i+1)) in self.multi_reso_active:
            # if True:
                o = self.post_nets_seg[f"disc_{i}"](x)
                print(i, o.shape)
                outs.append(o)
        in_utt = self.blocks["utt"](x)
        o_utt = self.post_nets_utt(in_utt)
        print('utt', o_utt.shape)
        if "utt" in self.multi_reso_active:
            outs.append(o_utt)
        return outs[0] if len(outs) == 1 else outs
