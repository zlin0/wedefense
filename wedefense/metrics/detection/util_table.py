...
Copied from
https://github.com/nii-yamagishilab/project-NN-Pytorch-scripts/blob/master/tutorials/plot_tools/table_API.py
"""
...
try:
    import matplotlib
    color_funcs = matplotlib.colormaps
...
# color function
#####################
def return_greys_tuple(value):
    return (1.0 - value, 1.0 - value, 1.0 - value, 0.0)
...
def return_latex_color_cell(value, val_min, val_max, scale, wrap, color_func):
    ...
    if scale < 0:
        value = wrap_value(
            (value - val_min) / (val_max - val_min), wrap) * -scale
        value = -scale - value
    else:
        value = wrap_value(
            (value - val_min) / (val_max - val_min), wrap) * scale
...
def print_table(data_array,
    ...
    if pad_data_column < 0:
        ...
        data_display_array = np.concatenate([dummy_col, data_display_array],
                                            axis=1)
    elif pad_data_column > 0:
        column_tag = ["" for x in range(pad_data_column)] + column_tag
        dummy_col = np.zeros([data_array.shape[0], pad_data_column]) + np.nan
        data_array = np.concatenate([data_array, dummy_col], axis=1)
        data_display_array = np.concatenate([data_display_array, dummy_col],
                                            axis=1)
    else:
        pass
    ...
    # color configuration
    if color_funcs:
        color_func = color_funcs[colormap]
    else:
        color_func = return_greys_tuple
    ...
    def get_latex_color(data_array, row_idx, col_idx, color_minmax_in):
        ...
        return return_latex_color_cell(x, value_min, value_max, colorscale_tmp,
                                       colorwrap, color_func)
    ...
    if pad_dummy_col > 0:
        ...
        latex_buffer += ''.join(['c'
                                 for x in range(pad_dummy_col)]) + r"}" + "\n"
    else:
        latex_buffer += r"\begin{tabular}{" \
    ...
    for row_idx in np.arange(row):
        ...
        row_content_text = [
            fill_cell(row_tag[row_idx], row_tag_max_len, col_sep)
        ]

        if pad_dummy_col > 0:
            row_content_latex = [
                fill_cell("", 1) for x in range(pad_dummy_col)
            ] + row_content_latex
        ...
        for col_idx in np.arange(col):
            ...
            if is_valid_float(data_array[row_idx, col_idx]):
                if len(tmp_print_format):
                    num_str = "{num:{form}}".format(num=data_array[row_idx,
                                                                   col_idx],
                                                    form=tmp_print_format)
                else:
                    num_str = ""
                latex_color_cell = get_latex_color(data_array, row_idx,
                                                   col_idx, color_minmax_in)
...
def concatenate_table(table_list,
                      ignore_initial=True,
                      add_separator=1,
                      latex=True):
    """
    ...
    # latex head
    latex_buffer = r"\begin{tabular}{" + ''.join(['c' for x in temp + ['']
                                                  ]) + r"}" + "\n"
