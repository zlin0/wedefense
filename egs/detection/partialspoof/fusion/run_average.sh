...
# Since here we evaluate partialspoof (PS), using EMBEDDING EXTRACTORS trained on asvspoof5 (A5) will result in cross-database fusion. Note that this refers to
# embedding extractor training data. THE CALIBRATION / FUSION MODELS ARE ALWAYS TRAINED ON INDOMAIN (PS) IN THESE EXPERIMENTS.
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    python wedefense/postprocessing/train_calibration_fusion_model_2.py \
       --score_dir $exp_dir/input_scores/dev/ \
       --cm_key ${data}/dev/cm_key_file.txt  \
       --model_save_path ${exp_dir}/min_max_avg.npy \
       --p_tar $p_tar \
       --c_fr $c_fr \
       --c_fa $c_fa \
       --method min_max_avg
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
	...
	python wedefense/postprocessing/apply_calibration_fusion_model_2.py \
               --model_load_path ${exp_dir}/min_max_avg.npy \
