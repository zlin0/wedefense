...
def calculate_minDCF_EER_CLLR_actDCF(cm_scores,
                                     cm_keys,
                                     output_file,
                                     printout=True):
    """
    ...
    # min DCF
    minDCF_cm, _ = compute_mindcf(frr, far, thresholds, Pspoof,
                                  dcf_cost_model['Cmiss'],
                                  dcf_cost_model['Cfa'])
    # actual DCF
    actDCF, _ = compute_actDCF(bona_cm, spoof_cm, Pspoof,
                               dcf_cost_model['Cmiss'], dcf_cost_model['Cfa'])
...
def calculate_aDCF_tdcf_tEER(cm_scores,
                             asv_scores,
                             sasv_scores,
                             cm_keys,
                             asv_keys,
    ...
    if asv_scores_org is None or asv_keys_org is None:
        ...
        # get the error rates
        tmp = obtain_asv_error_rates(tar_asv_org, non_asv_org, spf_asv_org,
                                     asv_org_threshold)
    ...
    # for a-DCF
    cost_adcf = a_dcf.CostModel(Ptrg=tdcf_cost_model['Ptar'],
    ...
    adcf = a_dcf._calculate_a_dcf(tar_sasv, nontar_sasv, spoof_sasv, cost_adcf,
                                  False)['min_a_dcf']
    ...
    # asv-cm score pairs
    X_tar = np.array([
        asv_scores[asv_keys == util.g_asv_tar],
        cm_scores[asv_keys == util.g_asv_tar]
    ], dtype=object)
    X_non = np.array([
        asv_scores[asv_keys == util.g_asv_non],
        cm_scores[asv_keys == util.g_asv_non]
    ], dtype=object)
    X_spf = np.array([
        asv_scores[asv_keys == util.g_asv_spf],
        cm_scores[asv_keys == util.g_asv_spf]
    ], dtype=object)
    ###
    ...
    Pmiss_ASV, Pfa_non_ASV, Pfa_spf_ASV, tau_ASV = compute_Pmiss_Pfa_Pspoof_curves(
        X_tar[0], X_non[0], X_spf[0])

    # Obtain CM error curves and CM thresholds.
    Pmiss_CM, Pfa_CM, tau_CM = compute_det_curve(
        np.concatenate([X_tar[1], X_non[1]]), X_spf[1])

    teer = compute_teer_accelerated(Pmiss_CM, Pfa_CM, tau_CM, Pmiss_ASV,
                                    Pfa_non_ASV, Pfa_spf_ASV, tau_ASV)
