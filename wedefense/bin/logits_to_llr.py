...
def main(logits_scp_path, training_counts, train_label, pi_spoof):
    ...
    training_counts_info = np.genfromtxt(training_counts, dtype=str, delimiter=" ")    
    ...
    ll_bonafide = logsumexp(logits[:,bonafide_idx] - np.log(np.array(bonafide_priors)), axis=1)
    ll_spoof    = logsumexp(logits[:,spoof_idx] - np.log(np.array(spoof_priors)), axis=1) 
    ...
    if len(nan_idx[0]):
        #print( "Warning: {} LLRs where nan! Set to mean of remaining LLRs".format(len(nan_idx[0])) )  
    ...
    #with kaldiio.WriteHelper('ark,scp:' + out_path + "/llr.ark," + out_path + "/llr.scp") as writer:
