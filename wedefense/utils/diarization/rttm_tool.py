...
def get_rttm(rttm_file):
    ...
    Args:
        ...
        SPEAKER <reco_id> <channel> <start_time> <duration> <NA> <NA> <label> <NA> <NA>   # noqa
...
def rttm2vadvec(rttm, seg_shift_sec, label2id, dur=0, skip_last=False):
    ...
    for lab, st, et in rttm:
        sf = max(0, int(st / seg_shift_sec +
                        0.5))  # To make sure at least one frame
...
def vadvec2othlab(vadvec, othlab, othlab2num, label2num_file):
    ...
    att2othlab = dict([[
        line.strip('\n').split(',')[0],
        line.strip('\n').split(',')[labidx[othlab]]
    ] for line in open(spf_detail_file)])
    newlab = [
        othlab2num[att2othlab[num2label[i]]]
        for i in vadvec.astype(int).astype(str)
    ]
...
def savepred_as_seg(args, res):
    ...
    f_segment = open(args.out_dir + '/res.segment',
                     'wt')  # <seg> <reco> <st> <end>
