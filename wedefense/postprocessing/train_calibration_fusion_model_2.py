...
def estimate_min_max_fusion(train_set):
    ...
    # The above is for all scores. For their average, we can combine their offsets into one 
    ...
    print("Estimated affine transform: scales {}, offset {}".format(aff_trans[0:-1], aff_trans[-1]))
    return aff_trans


def main(score_dir, cm_key, model_save_path, p_tar=0.5, c_fr=1, c_fa=1, method='LR'):
    ...
    if method == 'LR':
        print("Estimating calibration / fusion parameters with Logistic regression")
        ...
        print("Estimating calibration / fusion parameters with min-max (0-1) normalization")
        aff_trans = estimate_min_max_fusion(train_set=df[:,0:-2])  # The last two columns are label and weight which are not needed for this method
