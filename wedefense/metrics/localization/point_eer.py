...
def process_score_file(score_file, gt_exist=True, 
    ...
    if gt_exist:
        ...
        # col_names = ['reco', 'frame', 'bonafide_score', 'spoof_score'] + (['label'] if gt_exist else [])
        ...
        bona_cm = df[df['label'] == str(bonafide_idx)]['bonafide_score'].to_numpy()
        spoof_cm = df[df['label'] == str(spoof_idx)]['bonafide_score'].to_numpy()
    else:
        bona_cm = []
        spoof_cm = []
...
def main(score_file, score_reso, eval_label=None, gt_exist=True,
         label2id_file='data/partialspoof/train/label2id', printout=True) -> None:
    """
    ...
    if not (gt_exist or (eval_label and os.path.isfile(eval_label))):
        raise ValueError("Either score_file must include groundtruth labels or \
