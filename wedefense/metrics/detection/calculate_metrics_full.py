    ...
    for idx2, codec in enumerate(codec_list):
        ...
        for idx1, attack in enumerate(attack_list):
            ...
            if attack == util.g_pooled_label:
                # pooled over attacks
                query = '{:s} == "{:s}"'.format(util.g_cm_label_tag,
                                                util.g_cm_spf)
            else:
                # a specific attack
...
def calculate_aDCF_tdcf_tEER_full(sasv_score_key_pd,
                                  asv_org_score_key_pd,
                                  flag_sasv_only=False):
    """
